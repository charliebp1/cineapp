...
import com.cbritosp.app.model.*;
import com.cbritosp.app.service.*;
import com.cbritosp.app.util.Utileria;
...
public class HomeController {
	...
	public String mostrarPrincipal(Model model) {	
		...
		try {
			Date fechaBuscar = dateFormat.parse(dateFormat.format(new Date()));
			List<String> listaFechas = Utileria.getNextDays(4);		
			List<Pelicula> peliculas = servicePeliculas.buscarPorFecha(fechaBuscar);			
	...
	public String buscar(@RequestParam("fecha") Date fecha, Model model) {		
		try {			
			Date fechaBuscar = dateFormat.parse(dateFormat.format(fecha));
			List<String> listaFechas = Utileria.getNextDays(4);
			List<Pelicula> peliculas  = servicePeliculas.buscarPorFecha(fechaBuscar);
			model.addAttribute("fechas", listaFechas);			
